...
namespace tensorflow {
  ...
  public :
    ...
    void Compute(OpKernelContext* ctx) override {
      ...
      OP_REQUIRES_OK(ctx, GetWindowedOutputSizeVerboseV2(input_rows, filter_rows, dilation_rows, stride_rows, padding_type_, &output_rows, &padding_left, &padding_right));
      int64 padding_rows = padding_left + padding_right;
      
      int64 output_cols = 0, padding_top = 0, padding_bottom = 0; 
      OP_REQUIRES_OK(ctx, GetWindowedOutputSizeVerboseV2(input_cols, filter_cols, dilation_cols, stride_cols, padding_type_, &output_cols, &padding_top, &padding_bottom));
	...
	if (padding_type_ == Padding::SAME) {
	  ...
	  if (rows_odd || cols_odd) {
	    ...
	    Tensor padded_input;
	    TensorShape new_input_shape = ShapeFromFormat(data_format_, batch_size, new_input_rows, new_input_cols, input_channels);
	    OP_REQUIRES_OK(ctx, ctx->allocate_temp(DataTypeToEnum<T>::value, new_input_shape, &padded_input));
	...
	if (data_format_ == FORMAT_NHWC) {
	  ...
	  OP_REQUIRES_OK(ctx, ctx->allocate_temp(DataTypeToEnum<T>::value, nchw_shape_input, &transformed_input));
	  ...
	  OP_REQUIRES_OK(ctx, ctx->allocate_temp(DataTypeToEnum<T>::value, nchw_shape_output, &transformed_output));
	...
	if (filter_format_ == FORMAT_HWIO) {
	  ...
	  TensorShape oihw_shape_filter = ShapeFromFilterFormat(FORMAT_OIHW, filter.shape(), FORMAT_HWIO);
	  OP_REQUIRES_OK(ctx, ctx->allocate_temp(DataTypeToEnum<T>::value, oihw_shape_filter, &transformed_filter));
