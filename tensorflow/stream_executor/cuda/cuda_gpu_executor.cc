...
port::Status GPUExecutor::Init(int device_ordinal,
  ...
  status = GPUDriver::CreateContext(device_ordinal_, device_, device_options, &context_);
  if (!status.ok()) {
    return status;
  }
...
bool GPUExecutor::LoadModuleFromCuBin(const char *cubin, CUmodule *module) {
  ...
  if (*module == nullptr) {
    if (!GPUDriver::LoadGPUBinary(context_, GPUDriver::GPUBinaryType::CUDA_CUBIN, cubin, module)) {
...
bool GPUExecutor::LoadModuleFromPtx(const char *ptx, CUmodule *module) {
  ...
  if (*module == nullptr) {
    if (!GPUDriver::LoadGPUBinary(context_, GPUDriver::GPUBinaryType::CUDA_PTX, ptx, module)) {
      return false;
    }
...
int GPUExecutor::CalculateOccupancy(
