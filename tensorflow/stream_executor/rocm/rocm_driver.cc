...
ScopedActivateContext::ScopedActivateContext(int device_ordinal) {
  ...
  CHECK_EQ(hipSuccess, tensorflow::wrap::hipSetDevice(device_ordinal));
...
ScopedActivateContext::~ScopedActivateContext() {
  ...
  CHECK_EQ(hipSuccess, tensorflow::wrap::hipSetDevice(to_restore_));
...
/* static */ bool ROCMDriver::AsynchronousMemsetUint32(int device_ordinal,
  ...
  assert (value32 == value); // if mismatch this indicates case where hipMemsetAsyc can't emulate hipMemSetD32
  hipError_t res =
      tensorflow::wrap::hipMemsetAsync(pointer, value, uint32_count*4, stream);
  if (res != hipSuccess) {
    LOG(ERROR) << "failed to enqueue async memset operation: " << ToString(res);
    return false;
  }
...
/* static */ bool ROCMDriver::AddStreamCallback(int device_ordinal,
                                                ...
                                                void *data) {
  hipError_t res = tensorflow::wrap::hipStreamAddCallback(stream, (hipStreamCallback_t) callback, data, 0 /* = flags */);
  if (res != hipSuccess) {
...
/* static */ bool ROCMDriver::CreateStream(int device_ordinal,
                                           hipStream_t *out) {
  ScopedActivateContext activated{device_ordinal};
  hipError_t res = tensorflow::wrap::hipStreamCreateWithFlags(out, hipStreamDefault);  // switch to hipStreamNonBlocking?
  if (res != hipSuccess) {
...
/* static */ bool ROCMDriver::HostRegister(int device_ordinal, void *location,
  ...
  // "Portable" memory is visible to all ROCM contexts. Safe for our use model.
  hipError_t res =
      tensorflow::wrap::hipHostRegister(location, bytes, hipHostRegisterPortable);
  if (res != hipSuccess) {
...
/* static */ bool ROCMDriver::AsynchronousMemcpyH2D(int device_ordinal,
                                                    ...
                                                    hipStream_t stream) {
  ScopedActivateContext activation{device_ordinal};
  hipError_t res = tensorflow::wrap::hipMemcpyHtoDAsync(gpu_dst, const_cast<void*>(host_src), size, stream);
  if (res != hipSuccess) {
    LOG(ERROR) << port::Printf(
...
/* static */ port::Status ROCMDriver::CreateEvent(int device_ordinal,
  ...
  ScopedActivateContext activated{device_ordinal};
  hipError_t res = tensorflow::wrap::hipEventCreateWithFlags(result, hipflags);
...
/* static */ bool ROCMDriver::GetGridLimits(int *x, int *y, int *z,
  ...
  *x = value;
  ...
  res =
      tensorflow::wrap::hipDeviceGetAttribute(&value, hipDeviceAttributeMaxGridDimY, device);
  if (res != hipSuccess) {
  ...
  *y = value;
  ...
  res =
      tensorflow::wrap::hipDeviceGetAttribute(&value, hipDeviceAttributeMaxGridDimZ, device);
  if (res != hipSuccess) {
...
/* static */ port::Status ROCMDriver::EnablePeerAccess(int from_device_ordinal,
  ...
  hipError_t result = tensorflow::wrap::hipDeviceEnablePeerAccess(to_device_ordinal, 0 /* = flags */);
  if (result != hipSuccess &&
